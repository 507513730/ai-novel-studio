...
// 纪律：5s 超时、失败静默返回空、模块级缓存（TTL 1h）——离线环境零影响
...
/** Wikipedia 搜索（zh 优先；srlimit=4）——零 key */
async function searchWikipedia(query: string, lang: string): Promise<WebSearchResult[]> {
  ...
  const hits = ((data as { query?: { search?: Array<{ title: string; snippet: string }> } })?.query?.search ?? []).map(
    (s) => ({
...
async function fetchExcerpt(title: string, lang: string): Promise<string> {
  ...
  const data = await fetchJson(url)
  const pages = (data as { query?: { pages?: Record<string, { extract?: string }> } })?.query?.pages ?? {}
...
export async function searchWeb(db: DatabaseSync, query: string): Promise<{ results: WebSearchResult[]; excerpt: string }> {
  ...
  // 中文无结果时英文兜底
  const results = zh.length > 0 ? zh : await searchWikipedia(q, 'en')
  let excerpt = ''
  if (results.length > 0) {
