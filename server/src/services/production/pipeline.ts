// 整本生产主循环（重构计划 R4.3 / spec §3.4）：按章 生成 → 审核 → 低分修复（patch_first）→ 回灌。
// 只调用统一章节生成域（generateChapter），不自行解释章节抢占或重复正文落库逻辑；
...
import { selectPendingChapters, isGenerationSubstandard, isBatchFatalError } from './chapterPolicy'
import { createProgress, type ProductionProgress } from './progress'
...
export async function runProductionPipeline(
  ...
  opts: ProductionPipelineOptions = {}
): Promise<ProductionProgress> {
  const chapters = selectPendingChapters(db, novelId, opts)

  const progress = createProgress(novelId, chapters.length)
  onProgress(progress)
  ...
  enqueueAutoDebtFix(db, novelId, opts.jobId)
  return progress
}

// ---------- 单章审核 → 低分修复（patch_first）→ 写回 → 回灌 ----------
async function reviewFixBackfill(
  db: DatabaseSync,
  novelId: number,
  chapterId: number,
  ...
  onProgress(progress)
  const reviewMessages = buildChapterReviewContext(db, novelId, chapterId, generatedContent)
  ...
  if (isFixWarranted(review.score, review.issues)) {
    ...
    try {
      ...
      const patchMessages = buildPatchContext(db, novelId, chapterId, generatedContent, review.issues)
      ...
      const patched = applyPatches(generatedContent, patchResult.patches)
      if (patched !== null) {
        finalContent = patched
      } else {
        // 3b. 局部补丁失败（target 未匹配）→ 降级整章重写
        const fixMessages = buildFixContext(db, novelId, chapterId, generatedContent, review.issues)
      ...
      // 修复后重审（重审闭环）
      const rescoreMessages = buildChapterReviewContext(db, novelId, chapterId, finalContent)
  ...
  try {
    ...
    console.warn(`[production] 回灌失败（章节 ${chapterId}，评分 ${score}）: ${err instanceof Error ? err.message : String(err)}`)
  }
  return { score }
}

async function backfillChapterState(
  db: DatabaseSync,
  novelId: number,
  chapterId: number,
  finalContent: string
): Promise<void> {
  const backfillMessages = buildBackfillContext(db, novelId, chapterId, finalContent)
...
function enqueueAutoDebtFix(db: DatabaseSync, novelId: number, jobId?: number): void {
  try {
    if (getAutoFixEnabled(db) && jobId) {
      const pending = db
        .prepare(
