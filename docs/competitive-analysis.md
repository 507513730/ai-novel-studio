...
| # | 差距 | 竞品参照 | 现状与落点 |
|---|---|---|---|
| A1 | **多候选分支生成**：章节/续写一次生成 2-3 条走向并排对比，选定后落库，其余可存为版本 | 彩云小梦（核心体验）；Sudowrite Write | 现为单路径 + "再生成"；版本机制已具备存储基础，落点 = ChapterExecutionPage 生成动作 + Ctrl+J 续写浮层 |
| A2 | **快捷词/文本扩展**：`;zn`→主角名等码字宏，用户可自定义词典 | 阅文"快捷词/快捷标点"（网文作者刚需） | 快捷键系统仅 6 个动作（shortcuts.ts），无文本扩展；落点 = CodeMirror 补全插件 + 设置页词典管理 |
| A3 | **写作目标与统计面板**：日更目标/字数趋势/连击天数/写作时长 | Dabble goals；阅文码字统计 | AI/人工字数已分离、usage_log/chapter 字数数据齐备，仅缺汇总 UI（backlog 已列"未排期"，本报告并档） |
| A4 | **轻量本地校对**：错别字/重复用词/称谓一致性，随时可跑、不烧 token（或单次 extraction） | 阅文"精准纠错"；AI_NovelGenerator"自动校对" | 只有生成时三方会审；缺独立校对入口与一键定位 |
| A5 | **DOCX 导出 + 导出预览** | 投稿/平台后台事实标准 | 仅 TXT/MD/EPUB；导出预览无代码落点（阅读视图 .prose 可复用） |
| A6 | **演示书 + 交互式引导**：预置一本跑完管线的小书供首启浏览 | NovelCrafter"上手最难"差评是前车之鉴；Dabble 以易用著称 | Onboarding 只配 Key；HelpPage 静态卡片；nextSteps 引导卡已有但无 demo 数据 |
| A7 | 细节打磨：文件**拖拽导入**（AssetCreator）、主题**跟随系统**档 | 通用桌面惯例 | AssetCreator 仅按钮+粘贴（全库唯一拖拽是方案步骤排序）；7 套主题均为手动选择 |

### B 档 · 结构性功能（中成本、多家竞品共同验证）

| # | 差距 | 竞品参照 | 现状与落点 |
|---|---|---|---|
| B1 | **RAG/词条触发注入接入生成上下文**：前文命中关键词→注入对应设定条目；超窗后检索召回 | NovelAI Lorebook（关键词触发）；AI_NovelGenerator / Long-Novel-GPT / Writer 全部以检索注入保一致性 | TF-IDF Retriever + Embedding 预留接口已实现（D51）但**未接入**上下文组装——调研对象中一致性机制的最大共识差距；整书直塞在 1M 窗口内成立，超窗后这是唯一出路 |
| B2 | **写法示例动态检索**：作者已写正文入库，生成时检索相似片段作 few-shot 写法示例 | NovelWriter_public 招牌功能 | 写法资产为静态指纹/样本（styleFingerprint）；与 B1 同基建（已写章节入检索库） |
| B3 | **存量书稿接续创作**：导入作者手上的连载稿→自动提取卷/章/角色/账本→转为可继续写的工作书 | Sudowrite"导入已有小说自动建 Story Bible"；阅文"智能提取" | 拆书管线已有解析/五维分析/角色提取能力，但产物是"外部书"，不能作为工作书续写——补"转为工作书"转换层 |
