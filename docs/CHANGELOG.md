...
当前为 v1.1.3 候选，尚未正式发布；最近正式版本为 v1.1.2。

- 章节保存增加原文条件提交与持久请求回执，防止响应丢失重试重复记账；切章等待最新输入保存，冲突人工稿可另存版本后载入最新正文。版本恢复、候选采用和 AI 迟到响应增加会话归属保护，生成冲突保留候选并停止自动覆盖。

- 章节写作页收敛为正文优先布局，右栏分为写作/检查/版本；保留主题与字体设置，支持窄窗口抽屉、专注按钮、选区 AI 预览及原章待采用标记。
- 保存串行提交，确认成功才扣除字数增量；在途新输入保留未保存状态，空正文保护不再解除离开提醒。AI 结果核对原章原文后采用；修复遇到已保存正文冲突时保留候选版本，避免覆盖手改。

- 备份恢复先在独立暂存库校验完整性与应用结构、试迁移旧格式；拒绝损坏和未来版本库。替换时保留原库，新服务暂停任务并确认就绪后才提交；失败或中断可回退，保留最近一次成功恢复前副本。
- 补充恢复文件故障、真实进程 kill、启动失败回退及暂停任务测试；修正架构路径、检索说明与备份指南。经用户批准，统一动态产物名称、恢复安全约束、按 SHA 验证 CI、正式 tag 不变性与 pnpm 命令。
- 安全门禁最小覆盖 js-yaml 4.3.2、xmldom 0.8.15、sharp 0.35.4，清除本次审计的高危告警；中危与正式发布边界见 D143 和本轮审查记录。以上均尚未发布。

- 运行时传递依赖 qs 统一覆盖到 6.16.0，修复 GHSA-4mjr-xmp4-gh2g 与 GHSA-x5fp-wj9c-mxmx；增加 Express/body-parser 实际依赖路径回归。尚未发布。

## v1.1.2（2026-09-05）

### 写作偏好保存修复
- 快捷词编辑改由 `useWritingSettings` 作为唯一持久化入口，修复一次保存发出两次 PATCH、第二次失败误报及本地词典不同步。
- 保存期间锁定快捷词输入和写作偏好选项；同一事件循环的重复操作被同步锁拒绝。仅在服务端保存成功后清空快捷词草稿。
- 写作偏好加载/保存补充 5 项组件与 hook 回归：重复保存、失败重试、禁用状态、卸载迟到响应和 StrictMode 旧加载响应。

### 流程与文档纠偏
- 更正协作者手册、计划、发布流程和审查记录：v1.1.1 已正式发布，后续源码必须使用新 PATCH 候选；保留 v1.1.0 失败 tag 的历史事实。
- 记录并修复 b64bf79 未 bump 导致的 Release Readiness 失败，候选版本与正式 Release 状态不再混用。

## v1.1.1（2026-09-05）

### 累积交付范围
- 本候选同时包含未成功发布的 v1.1.0 源码功能：导出预览、多候选分支、词条触发检索、已写片段检索和存量书稿转工作书；具体功能说明保留在下方 v1.1.0 段。历史失败 tag 不重指。

### 备份安全修复
- 备份改由 utilityProcess 生成一致性快照，错误和超时不再假成功。
- 导出拒绝覆盖现有目录；自动轮转只删除受管文件，未完成快照禁止恢复。
- 修正 parentPort 命令封装与恢复目录误选父目录的问题。

### 构建依赖安全
- 将受影响的 fast-uri v3 传递依赖约束到 3.1.6，补齐全依赖高危审计，避免 prod-only 检查遗漏打包工具链。

### 流程与文档修复
- 正式发布强制完整证据、当前 SHA 的 CI 与全部平台资产；矩阵全部成功后才统一发布。
- 分离版本准备与发布，拒绝重复 bump、旧报告、部分 E2E、跳过打包后发布及重用 tag。
- 纠正 v1.1.0 被误标为已发布，移除删失败记录/重打 tag/明文测试的过时指引。
...
> 发布状态勘误（2026-09-05）：tag 已存在，但构建失败，GitHub Release 不存在。原变更内容保留作为源码历史。
