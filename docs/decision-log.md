...
> ⚠️ **历史乱码损坏（不可恢复）**：D80-D89 条目的中文在早期编码事故中被有损替换为 `?`（0x3F；字节级证实，首次引入提交即已损坏）。条目标题已按 CHANGELOG/提交信息重建；正文要点以 [CHANGELOG.md](CHANGELOG.md) 与代码为准。D90 起记录完好。

### D80 · 2026-08-12 · 查证收尾（补查 4 项 + D75 勘误）
???? Node IncomingMessage 'close'?????? v16.0.0 ?"???????"?? socket ????**D75 ?"Node24 ??"????? v16+ ????**????????res.on('close')+writableEnded ??????????'aborted' ?? v17 ????????? close??? stream_options.include_usage??OpenAI ????????"?????????? usage chunk"?????? abort ?????? openai-node ???? err.status/err.headers ???? RequestOptions ????**??? SDK ?????? 2 ??429/408/409/>=500?? callLlm ??? tryCount 3 ????? 9 ????? ? maxRetries:1**?? SQLite INSERT...SELECT WHERE NOT EXISTS ??????????????????AGENTS ?61 ????????

### D81 · 2026-08-12 · 批B（O5 成本预警月度预算 + I2 质量债自动修复闭环）
?? Anthropic Building Effective Agents???????????????? = ?? **evaluator-optimizer** ??????????????????????????? + ?????????????????? + ???????????**???????????? + ????**?"higher costs, potential for compounding errors, appropriate guardrails"????????? 2 ?????? P12 C1?+ ????? + ????? ?75 ??????? P1.5?+ ????? job ???????????????+ ??????????????O5??????????? + ??? + ???????????

### D82 · 2026-08-12 · GitHub Pages 方案市场（A 面查证）
2026-08-10 ???? github-pages ???????#5833018423????? A ??????? A/B ????????????gh api DELETE??deployments ?????????????????????????????????????????????????"??????"????

### D83 · 2026-08-12 · 批C npm manifest 校验（name+version 一致 / kebab id / description+tags）
?? npm package.json ???docs.npmjs.com??? name+version ???????**???????????**??????????? name+version ???????????? ??????description/keywords ? ???? description+tags?? name ??????URL ??????214 ? ?? id ??? kebab-case?? license/author ???????????????????????GitHub ?? solutions/ ?? + index.json ?? + raw.githubusercontent.com ???CORS ????????????????index.json ??????

### D84 · 2026-08-12 · 批C solution-pack 方案包（kind/id/version/metrics/sampleBook）
????????D82??D83 ???npm manifest ???name+version ????/?? kebab-case id/description+tags ????????solution-pack ???kind/id/version/metrics/sampleBook ???????? solution????? id ?? + hash ?????? = GitHub ?? solutions/ ???index.json ?? + raw ??????????????????????????/??????/????/??????????? scripts/export-market-pack.mjs?????????????+ publish-solution.mjs?????????????mc-good2.0 v1.0.0?10 ?????????

### D85 · 2026-08-12 · UI 体验修复（智能体删除 / 乱码清零 / 空状态引导卡片）v0.11.1
???? 4 ????? ???????????? agents.ts ? DELETE /:id ????????????????????? 409 ?? + ???? json_each ???? 409 + agent_skill ???????? instr ?????????????????????????? json_each ?????? ????/??/?????????3 ? EmptyState ?????????????? StudioPage ?? 6 option????????8 ??????? 10 ????? ?????????????? EmptyState ?? + ??????????Titles/BaseCharacters/BookAnalysis 3 ???? ???????????????????????????????

### D86 · 2026-08-12 · 批D P31 方案感知卷章（卷章定位注入整本生产）
?????P30 ???"?????? ? ?????????? + ?? + ????"?production.ts whole_book ????buildChapterWriteContext ?"????"????/??/???P31 ???? = **???? prompt ??????**???/??/?? + ???????????????? title/summary/goal????"??????????????"???????????????????????????? services/chapterRole.ts??getChapterPosition???/????/??/??/??????=????=????=???+ chapterPositionBlock????????????????????? runProductionChapter ??? prompt?

### D87 · 2026-08-12 · 批D 补查（文档断言 verify-docs + versioning 台账）
P31 ???????????chapterRole ?? + ?? prompt ??????????? mc-good2.0 ?????? 3 ???????????? outline???/??/???????vitest 92/92?Release v0.12.0??**??????**?versioning.md ?7 ???? v0.5.0 ??????????? 0.4.0???? A-D ? 8 ??? docs ?? replace ???????????????????release.mjs vitest ?????scripts???????? main?????????????? + ????? CHANGELOG ??????????? E?verify-docs.mjs ?????

### D88 · 2026-08-12 · 批E 世界状态机（势力状态 backfill + 时间线消费）I4
?? MemGPT/Letta???????? + ?????????????????+ main context????????????SQLite ????character.ledger/foreshadow/fact/timeline_event/world.factions?= ?????context ???? = ???????**????**???????????/??/??/????timeline_event ?????????????/??/????/???? desc?**I4 ????**?? ???????backfill ?? factionStates ? factions_json.currentState ? ??????????? ???????? 5 ?????????chapter_id ? ?????? ??????????production ??????????????????????verify-docs.mjs ???release.mjs [3/7]?+ versioning ?7 ???0.5.0?0.13.0?+ D87 ???

### D89 · 2026-08-12 · 批F 风格指纹（Stylometry 文体计量学）I5
?? Wikipedia Stylometry????? = ?????writer invariant?+ ????/?????"?????????????????????"?+ ??/?????**?????**??????????styleFingerprint.ts?computeStyleFingerprint ???? LLM?????/???/??????/??/????/????/??????+ fingerprintDescription????????+ ?? /style/fingerprint??????????????? ? style_asset?????????????????????????????????????+ Number(??) ????????????? 16.6/?? 40%/?? 24%??O1-O5 + I1-I5 ?????
...
- **乱码不可恢复（字节级证实）**：PLAN.md P18-P28 / §12 早期记录、decision-log D80-D89 的中文被有损替换为字面 0x3F；git 历史中 P18 段（54103e6 首次引入）与 D80（ef9e892 首次引入）即已乱码——损坏早于提交（PowerShell 编码事故，见教训②）。处理：banner 标注 + 标题按 CHANGELOG/提交信息重建，正文要点以 docs/CHANGELOG.md 为准。
- **顺带修复**：README 测试数过时（101→143，单测 73→143、db-smoke 6→7 项）；tests/p29.test.ts 7 处乱码描述重建（依据测试体）；D90 两处 0x07 控制符损坏（app.use / artifactName）与 “node -e” 丢字。
