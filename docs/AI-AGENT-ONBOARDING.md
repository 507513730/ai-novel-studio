...
- **当前版本**：`v1.1.2` 候选，以 package.json 为准；最近正式 Release 为 `v1.1.1`（2026-09-05，tag `59b5671`）。`v1.1.0` 只有失败构建对应的 tag；候选完成全部门禁和用户授权的发布流程前不得称为已发布（见 D138-D139）。
...
- 开发导航按准确 HTTP(S) origin 比较，打包导航限定 renderer 入口文件；不以字符串前缀或任意 file URL 作为白名单。
...
- **排期**：UI 全面升级收官批（批次 A → v0.26.0、批次 B → v1.0.0，问题清单见 docs/ui-review.md）→ 1.0 发布（2026-08-29 用户裁定，见 versioning §1.1）；写书在 1.x 内收官（功能请求以用户实际要求为准）。
