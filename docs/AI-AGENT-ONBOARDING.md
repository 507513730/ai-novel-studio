...
- **当前版本**：`v0.22.2`（以 package.json 为准；发布经 GitHub Release + 应用内自更新）。
...
server/src/         服务进程（Node 隔离）：routes/（~17 路由）services/（业务）db/（迁移+种子）prompts/
  services 关键文件：generate / context（前缀冻结） / llm（路由+降级+记账） / planner（导演 prompt 统一）
                    / ledger（状态账本） / jobQueue / scheduler / director / production / solutionRunner
                    / debtFix / tripleReview / retrieval（TF-IDF） / styleEngine / smartContext
                    / constraintEngine / settingBrief / webSearch / currency / keyCrypto / security
