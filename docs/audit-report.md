...
| # | 严重度 | 问题 | 位置 | 所属批次 | 状态 |
|---|---|---|---|---|---|
| R0-F1 | P1 | 反 AI 重写成功后二次 UPDATE 只改 chapter 不更新 chapter_version → 正文与最新版本快照分叉（违反「版本=正文快照」契约） | generate.ts:197-199 vs :127-131 | R1 | [ ] |
| R0-F2 | P2 | 章节内容/状态 UPDATE 缺 novel_id 守卫（其余同类路径均带；章 id 全局唯一，实际风险低，属一致性修复） | generate.ts:137/198/240 | R1 | [ ] |
| R0-F3 | P3 | 注释声称「最终内容（含反 AI 重写后）确定性校验」，代码实际在重写前校验（注释与顺序不符） | generate.ts:151-153 | R1 | [ ] |
| R0-F4 | P2 | 参考分支 postProcess 约束登记取「该书最后一章」id 而非被生成章节 id——重放时必须改为传入 chapterId，不得回退 main 行为 | 旧分支 chapterGeneration/postProcess.ts:21 | R1 | [ ] |
