# AI-Novel-Studio 内容说明（归档头）

> 本文件为历史归档（2026-08-22 文档治理归档）：原 `PLAN.md` 完整实施编年史（v3.1 审查修订版 + P0-P30+ 各阶段清单 + §12 版本记录）。
> **当前状态以仓库根 `PLAN.md`（精简当前版）为准**；本文件保留完整历史与部分早期编码事故损坏段（0x3F 不可恢复，已标注；该类段落要点以 git 提交信息与已发布代码为准）。
>
> 后续新任务：请使用根目录 `PLAN.md`（当前版）；需要查历史决策/阶段清单时再查本文件与 docs/decision-log.md / docs/CHANGELOG.md。

---

（以下为归档时的原 PLAN.md 全文，未做内容修改）
