...
2026-09-12（D145-D146）：按用户批准的设计改造章节写作页，拆出工作台编排与保存 hook；右栏写作/检查/版本、保存状态、选区 AI 预览与原章归属保护。补充正文条件提交、持久保存回执和冲突草稿处理。继续使用未发布 v1.1.3 候选；验证与产物记录见 [章节写作页验收](docs/development/chapter-workspace-2026-09-12.md)。

2026-09-11（D144）：用户批准准备 v1.1.3 候选、提交推送并核验源码 CI，4 项规范修订已生效。后续正文并发批遵循“保留用户输入，冲突 AI 结果留待采用版本”；正式 tag/Release 仍单独确认，既有 v1.1.2 为最新正式发布。

2026-09-11：全仓初筛进入恢复安全批次（D142）：暂存独立校验、旧格式试迁移、未来 schema 拒绝、原库保留、暂停启动确认及失败/中断回退。收尾发现依赖高危，按 D143 最小覆盖 js-yaml/xmldom/sharp，复审 0 high/critical（保留中危记录）。普通架构文档同步纠偏，硬规范修订已获批准并生效。代码与本地包尚未正式发布，v1.1.3 候选及远端闭环正在收尾；其余 9 项代码发现仍待分批处理，见 [本轮审查记录](docs/review-2026-09-11.md)。
