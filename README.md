...
- `pnpm test`：vitest 单测（**数量以命令输出为准**，随版本增长）。覆盖补丁修复/导演/SSE 取消/成本估算/模型覆盖/世界渲染/方案资产/引导系统/创作约束/记忆面/审查回归/字数覆盖语义/UI 主题/版本 diff/全文检索/安全加固；v0.25.0 起新增 **React 组件测试**（章节面板 + 页面级错误边界，此前前端零覆盖）
  - 组件测试位于 `tests/*.test.tsx`，文件首行以 `// @vitest-environment jsdom` 声明
