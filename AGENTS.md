...
3. **pnpm 专用**：禁止 npm/yarn。`.npmrc` 已配 `store-dir=D:\.pnpm-store` + electron 镜像。
...
- 项目根：`D:\OpenCode\projects\ai-novel-studio`（非 git 仓库）
- 测试凭证：OpenCode Go 网关 key（`~/.local/share/opencode/auth.json`）

41. **?????**?P13 F0/D36???????????? CSS ?????? 4 ?/?? 3 ?/accent ?/???/???????????? nativeTheme + titleBarOverlay?IPC theme-set??CodeMirror ????? CSS ???
42. **???????**?P13 G1/D37??retry ? modelOverride ??????????payload ???????? scheduler ???????????????buildCandidates ?? override ????? fallback?
...
51. **????**?docs/versioning.md??SemVer ???MINOR=???/PATCH=?????? = bump package.json ? ?? ? tag vX.Y.Z ? push tag?CI ???? tag==version???? force tag / ???? tag??? Release ? versioning.md ?6?
52. **????**?P21?????? = solution ? steps_json?agentId/role/stage/include/maxTokens/if??stage ?? post_generate/review/whole_book??????????????????????????????????? hub ????
53. **Feelfish ??**?agent md ? YAML frontmatter ???name/description/tools/skills?????????agent ? name?skill ? name+novel_id=0??
54. **?????**?????????? /solutions/generate?? callLlmJson + schema ?? + ????????????? 90s ???????????? degradedReasons?

55. **????**??????????? `pnpm release`?scripts/release.mjs????? versioning.md ?3 ?????????????????? tag?**???????? `pnpm dist`**??? release/ ??? Release ??????????????
56. **????**???/??/?????? docs/README.md ?????????release-notes.md / PLAN.md / decision-log.md ?????????????? release-notes??

57. **????**?????/???? main ? bump ???? `pnpm release --push`???"?????"??? release/ ?? = ??????????????????? PLAN/decision-log?
58. **CI ????**????????????????workflow ??? if: false + ???????

59. **??????**????? **deepseek-v4-flash**????????????????k3/pro/glm/gpt ???????????????e2e/??/???? flash??????????????
