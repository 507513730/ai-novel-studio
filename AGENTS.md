...
60. **提交规范（开放仓库）**：commit 遵循 Conventional Commits——feat/fix/docs/refactor/test/chore/ci 等类型 + scope 可选（如 feat(p30)）；**标题与正文描述一律使用中文（2026-08-29 起，用户强制）——仅 Conventional Commits 的类型前缀保留英文**（如 `refactor: 章节生成域拆分`，commitlint 主题中英文均可）；破坏性变更必须标注 BREAKING CHANGE 页脚；PR 由 CI 检查（.github/workflows/commitlint.yml），本地不装钩子；commit 是公开历史，禁止空描述、禁止裸 commit 内容。
