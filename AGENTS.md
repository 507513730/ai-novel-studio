...
2. **版本锁定**（已逐一验证，禁止随意升级）：electron 43.3.0、electron-vite 5.0.0、vite **7.3.6（不可用 8）**、react 19.2.8、typescript **5.9.3**、express 5.2.1、zod 4.4.3、**openai SDK（直连，7.4.0）**、@tanstack/react-query 5.101.4、zustand 5.0.14、codemirror 栈（@uiw/react-codemirror ^4.25.11 + @codemirror/lang-markdown ^6.5.2）、react-router-dom ^7.9.0、lucide-react ^1.31.0（P11 引入，纯 JS 图标库）、electron-updater ^6.8.9（自更新）、epub2 ^3.0.2（拆书 EPUB 解析）、gpt-tokenizer ^3.4.0（客户端成本预估）、**epub-gen-memory 1.1.2（epub-gen 0.1.0 是 2019 死包，禁用）**、electron-builder 26.15.3。**禁止引入 @langchain/langgraph、@langchain/openai、@langchain/core**（审查结论：LangChain 对 DeepSeek 是负资产——reasoning_content 字段丢失 bug 未修，见 langchainjs #10883；改 openai SDK 直连）。
